...
function measureText(ctx, string) {
//	var textWidth = data[string];
//	if (!textWidth) {
//		textWidth = data[string] = ctx.measureText(string).width;
//		gc.push(string);
...
function computeLabelSizes(ctx, tickFonts, ticks) {
	var length = ticks.length;
	var widths = [];
	var heights = [];
	var i, j, jlen, tick, label, tickFont, width, height, nestedLabel, widest, highest;
	...
	for (i = 0; i < length; ++i) {
		...
		if (!helpers.isNullOrUndef(label) && !helpers.isArray(label)) {
			width = measureText(ctx, label);
			height = tickFont.size;
			...
			for (j = 0, jlen = label.length; j < jlen; ++j) {
				...
				if (!helpers.isNullOrUndef(nestedLabel) && !helpers.isArray(nestedLabel)) {
					width = Math.max(width, measureText(ctx, nestedLabel));
					height += tickFont.size * (j > 0 ? 1.25 : 1);
				}
			}
		}
		widths.push(width);
	...
	widest = widths.indexOf(Math.max(widths));
	highest = heights.indexOf(Math.max(height));
...
function parseFontOptions(options) {
	...
	return {
		...
		color: valueOrDefault(options.fontColor, globalDefaults.defaultFontColor)
	};
}
...
function parseTickFontOptions(options) {
	...
	var globalDefaults = defaults.global;
	var result = {};
	var size, style, family;

	['minor', 'major'].forEach(function(key) {
		size = valueOrDefault(valueOrDefault(options[key].fontSize, options.fontSize), globalDefaults.defaultFontSize);
		style = valueOrDefault(valueOrDefault(options[key].fontStyle, options.fontStyle), globalDefaults.defaultFontStyle);
		family = valueOrDefault(valueOrDefault(options[key].fontFamily, options.fontFamily), globalDefaults.defaultFontFamily);
		...
		result[key] = {
			...
			family: family,
			font: helpers.fontString(size, style, family),
			color: valueOrDefault(valueOrDefault(options[key].fontColor, options.fontColor), globalDefaults.defaultFontColor)
		};
	});
...
module.exports = Element.extend({
	...
	calculateTickRotation: function() {
		...
		var ticks = me._ticks;
		var labelSizes, widestLabelSize, highestLabelSize, tickWidth, angleRadians, cosRotation, sinRotation;

		// Get the width of each grid by calculating the difference
		// between x offsets between 0 and 1.
		var labelRotation = tickOpts.minRotation || 0;

		if (ticks.length && options.display && me.isHorizontal()) {
			labelSizes = computeLabelSizes(me.ctx, parseFontOptions(tickOpts), ticks/* , me.longestTextCache */);
			widestLabelSize = labelSizes.widest;
			...
			while (widestLabelSize.width > tickWidth) {
				angleRadians = helpers.toRadians(labelRotation);
				cosRotation = Math.cos(angleRadians);
				sinRotation = Math.sin(angleRadians);
				if (highestLabelSize.height <= tickWidth * sinRotation - 6 ||
					labelRotation >= tickOpts.maxRotation ||
					sinRotation * widestLabelSize.width + cosRotation * widestLabelSize.height >= me.maxHeight ||
					sinRotation * highestLabelSize.width + cosRotation * highestLabelSize.height >= me.maxHeight) {
					break;
				}
				labelRotation++;
			}
	...
	fit: function() {
		...
		if (tickOpts.display && display) {
			var tickFonts = parseFontOptions(tickOpts);
			var labelSizes = computeLabelSizes(me.ctx, tickFonts, me._ticks/* , me.longestTextCache */);
			var lineSpace = tickFonts.minor.size * 0.5;
			var tickPadding = tickOpts.padding;

			if (isHorizontal) {
				// A horizontal axis is more constrained by the height.
				me.labelSizes = labelSizes;
				...
				var labelHeight = Math.max(
					sinRotation * widestLabelSize.width + cosRotation * widestLabelSize.height,
					sinRotation * highestLabelSize.width + cosRotation * highestLabelSize.height)
					+ lineSpace; // padding
				...
				var labelWidth = tickOpts.mirror ? 0 :
					...
					labelSizes.widest.wifth + tickPadding + lineSpace;
	...
	_autoSkip: function(ticks) {
		...
		if (isHorizontal) {
			...
			var tickWidth = me.width / (tickCount || 1);
			if (labelSizes.widest.width + optionTicks.autoSkipPadding > cosRotation * tickWidth &&
				labelSizes.highest.height + optionTicks.autoSkipPadding < sinRotation * tickWidth) {
				skipRatio = 1 + Math.floor(Math.min(
					(labelSizes.widest.width + optionTicks.autoSkipPadding) * tickCount / (cosRotation * me.width),
					(labelSizes.highest.height + optionTicks.autoSkipPadding) * tickCount / (sinRotation * me.width)));
			}
	...
	draw: function(chartArea) {
		...
		if (scaleLabel.display) {
			...
			context.fillStyle = scaleLabelFont.color; // render in correct colour
...
module.exports = function() {
	...
	var TimeScale = Scale.extend({
		...
		tickFormatFunction: function(tick, index, ticks, formatOverride) {
			...
			var nestedTickOpts = major ? majorTickOpts : options.ticks.minor;
			var formatter = helpers.valueOrDefault(
				helpers.valueOrDefault(nestedTickOpts.callback, tickOpts.callback),
				helpers.valueOrDefault(nestedTickOpts.userCallback, tickOpts.userCallback)
			);
