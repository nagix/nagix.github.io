...
function parseFontOptions(options, nestedOpts) {
	var globalDefaults = core_defaults.global;
	var size = resolve$7([nestedOpts.fontSize, options.fontSize, globalDefaults.defaultFontSize]);
	var style = resolve$7([nestedOpts.fontStyle, options.fontStyle, globalDefaults.defaultFontStyle]);
	var family = resolve$7([nestedOpts.fontFamily, options.fontFamily, globalDefaults.defaultFontFamily]);
	...
	return {
		...
		lineHeight: helpers$1.options.toLineHeight(resolve$7([nestedOpts.lineHeight, options.lineHeight, globalDefaults.defaultLineHeight]), size),
		color: resolve$7([nestedOpts.fontColor, options.fontColor, globalDefaults.defaultFontColor])
	};
}
...
var resolve$8 = helpers$1.options.resolve;
...
function drawPointLabels(scale) {
	...
	if (ctx.setLineDash) {
		ctx.setLineDash(resolve$8([angleLineOpts.borderDash, gridLineOpts.borderDash, []]));
		ctx.lineDashOffset = resolve$8([angleLineOpts.borderDashOffset, gridLineOpts.borderDashOffset, 0.0]);
	}
